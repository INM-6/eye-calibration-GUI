...
% 			 H: 0.25
% 
% 	If TROUGH_LOCATION == 'estimate', then the input ESTIMATE_L2 must be
% 	provided, as per: 
% 	DATA_OUT = KINARM_ADD_TROUGH_INERTIA(DATA_IN..., 'trough_location', TROUGH_LOCATION, 'estimate_L2', ESTIMATE_L2 ...)
% 	If ESTIMATE_L2 == true, then the length of L2 is estimated from the
% 	length of L1, which is extracted on a per-trial basis from DATA_IN.
% 	Otherise L2 is extracted from DATA_IN.  L1 and L2 are then used with
% 	information in TROUGH_DB to estimate trough location.  See
% 	KINARM_CREATE_TROUGH_DATABASE for the required information in TROUGH_DB.
% 
...
if ischar(trough_location) && strcmp(trough_location, 'estimate')
	% if the function is supposed to estimate trough location, we need to
	% know if L2 should be estimated or not (i.e. it should have been
	% passed in as part of varargin) 
	if isempty(estimate_L2)
		error('---> Trough location is supposed to be estimated, but a valid input for estimate_L2 was not found.');
	end
